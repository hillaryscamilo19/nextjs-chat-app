...
export default function SimpleTestPage() {
  ...
  const testBackend = async () => {
    ...
    try {
      // Test 1: Health check
      const healthResponse = await fetch("http://10.0.0.15:3002/api/health")
      ...
      // Test 2: Register
      const registerResponse = await fetch("http://10.0.0.15:3002/api/auth/register", {
        method: "POST",
      ...
      // Test 3: Login
      const loginResponse = await fetch("http://10.0.0.15:3002/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: "test@example.com",
