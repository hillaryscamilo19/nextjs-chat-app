...
import { X, Users, Settings, Crown, Shield, UserMinus, UserPlus, Edit3, Save, LogOut } from "lucide-react"
...
export function GroupInfoModal({ isOpen, onClose, group, currentUser, onGroupUpdated }: GroupInfoModalProps) {
  ...
  const isAdmin = group.admins.some((admin) => admin._id === currentUser._id) || isCreator
  const canEditInfo = isAdmin && (group.settings.onlyAdminsCanEditInfo ? isAdmin : true)
  const canAddMembers = isAdmin && (group.settings.onlyAdminsCanAddMembers ? isAdmin : true)
  ...
  const handleToggleAdmin = async (userId: string, makeAdmin: boolean) => {
    ...
    try {
      ...
      toast({
        ...
        description: `El usuario ${makeAdmin ? "ahora es" : "ya no es"} administrador`,
      })
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const loadAvailableUsers = async () => {
    ...
    try {
      ...
      setAvailableUsers(users.filter((user) => !participantIds.includes(user._id)))
    } catch (error) {
      toast({
        title: "Error",
              ...
              <div className="flex-1">
                {isEditing ? (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">Creado por {group.createdBy.name}</p>
                  </div>
                )}
              </div>
              {canEditInfo && (
                <div className="flex gap-2">
                  {isEditing ? (
                    <>
                      <Button size="sm" onClick={handleSaveInfo} disabled={isLoading}>
                  ...
                  <div key={member._id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50">
                    ...
                    {isCreator && !isCurrentUser && !isMemberCreator && (
                      <div className="flex gap-1">
                        ...
                        >
                          <Shield className={`h-4 w-4 ${isMemberAdmin ? "text-orange-500" : "text-gray-400"}`} />
                        </Button>
                        <Button
                          size="sm"
                          variant="ghost"
          ...
          <div className="p-4 border-t">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                ...
                <Badge variant={group.settings.onlyAdminsCanMessage ? "default" : "secondary"}>
                  {group.settings.onlyAdminsCanMessage ? "Activado" : "Desactivado"}
                </Badge>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span>Solo admins pueden agregar miembros</span>
                <Badge variant={group.settings.onlyAdminsCanAddMembers ? "default" : "secondary"}>
                  {group.settings.onlyAdminsCanAddMembers ? "Activado" : "Desactivado"}
                </Badge>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span>Solo admins pueden editar información</span>
                <Badge variant={group.settings.onlyAdminsCanEditInfo ? "default" : "secondary"}>
                  {group.settings.onlyAdminsCanEditInfo ? "Activado" : "Desactivado"}
                </Badge>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60">
          <div className="bg-white rounded-lg w-full max-w-md mx-4 max-h-[70vh] flex flex-col">
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setShowAddMembers(false)}>
                <X className="h-5 w-5" />
              </Button>
            </div>
