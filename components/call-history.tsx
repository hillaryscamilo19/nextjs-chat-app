...
export function CallSystem({ conversation, currentUser, onCallActive }: CallSystemProps) {
  ...
  useEffect(() => {
    ...
    const handleIncomingCall = (data: any) => {
      if (data.conversation === conversation.id && data.initiator._id !== currentUser.id) {
        setActiveCall(data)
        setIsRinging(true)
        toast({
      ...
      <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-8 text-center max-w-sm w-full mx-4">
          ...
          <p className="text-gray-500 mb-6">
            {activeCall.type === "video" ? "Videollamada entrante" : "Llamada entrante"}
          </p>

          <div className="flex gap-4 justify-center">
            <Button onClick={declineCall} size="lg" className="rounded-full w-16 h-16 bg-red-500 hover:bg-red-600">
              <PhoneOff className="h-6 w-6" />
            </Button>
            <Button onClick={acceptCall} size="lg" className="rounded-full w-16 h-16 bg-green-500 hover:bg-green-600">
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-8 text-center max-w-sm w-full mx-4">
          ...
          <h3 className="text-xl font-semibold mb-2">{otherUser?.name}</h3>
          <p className="text-2xl font-mono font-semibold text-blue-500 mb-6">{formatCallDuration(callDuration)}</p>
          ...
          <Button onClick={endCall} size="lg" className="w-full rounded-full bg-red-500 hover:bg-red-600 text-white">
