...
export function PinnedMessages({
  ...
  const loadPinnedMessages = async () => {
    ...
    try {
      ...
      console.log("[v0] Loading pinned messages for:", conversationId);
      const messages = await pinnedMessagesAPI.getPinnedMessages(
        conversationId
      );
      setPinnedMessages(messages);
    } catch (error: any) {
      console.error("[v0] Error loading pinned messages:", error);
      toast({
        title: "Error",
        description:
          error.message || "No se pudieron cargar los mensajes fijados",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const unpinMessage = async (pinnedMessageId: string) => {
    try {
      ...
      setPinnedMessages((prev) =>
        prev.filter((msg) => msg._id !== pinnedMessageId)
      );
      toast({
              ...
              {isLoading ? (
                ...
                <div className="p-4 text-center">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Fija mensajes importantes para acceder rápidamente
                  </p>
                </div>
              ) : (
